-- Migration: Create payroll import history tables (Cloudinary storage version)
...
CREATE TABLE IF NOT EXISTS payroll_import_sessions (
    ...
    month_period VARCHAR(100),                  -- e.g. "Tháng 1/2025"
...
CREATE TABLE IF NOT EXISTS payroll_import_details (
    id SERIAL PRIMARY KEY,
    session_id INTEGER NOT NULL REFERENCES payroll_import_sessions(id) ON DELETE CASCADE,
    ...
    CONSTRAINT chk_import_detail_status CHECK (
        status IN ('sent', 'failed', 'not_found', 'no_gmail', 'limit_reached')
    )
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_import_sessions_imported_at ON payroll_import_sessions(imported_at DESC);
CREATE INDEX IF NOT EXISTS idx_import_details_session_id ON payroll_import_details(session_id);
CREATE INDEX IF NOT EXISTS idx_import_details_status ON payroll_import_details(status);
